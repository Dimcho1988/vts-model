...
import pandas as pd
from database import insert_workout, list_workouts, mark_workout_has_streams, get_token, insert_hr_speed_points
...
def fetch_activities(athlete_key: str, after_ts: int, before_ts: int,
    ...
    if not tok:
        raise RuntimeError("No token for athlete.")
    params = {"after": after_ts, "before": before_ts, "per_page": 100, "page": 1}
    ids = []
    while True:
        r = requests.get(f"{BASE}/athlete/activities", headers=_headers(tok["access_token"]), params=params, timeout=30)
        r.raise_for_status()
        arr = r.json()
        if not arr:
            break
        for a in arr:
            if a.get("type") not in ("Run","TrailRun"):
                continue
            start_iso = datetime.fromisoformat(a["start_date"].replace("Z","+00:00")).astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
            duration_s = float(a.get("elapsed_time", 0))
            ...
            strava_id = int(a["id"])
            try:
                insert_workout(athlete_key, start_iso, duration_s, distance_m, avg_hr, avg_speed, a.get("name"), strava_id)
            except Exception:
                pass
...
def fetch_streams_for_activity(athlete_key: str, strava_id: int,
    ...
    r = requests.get(f"{BASE}/activities/{strava_id}/streams", headers=headers, params={"keys": keys, "key_by_type": "true"}, timeout=60)
    r.raise_for_status()
    js = r.json()
    ...
    if attach_workout_id is None:
        ...
        if wid is None:
            ...
            wid = insert_workout(athlete_key, start_iso, 0.0, 0.0, None, None, f"Auto from streams {strava_id}", strava_id)
    else:
        wid = attach_workout_id
    pts = [{"t_bin_start": r["t_bin_start"],
    ...
    ins = insert_hr_speed_points(wid, pts)
    mark_workout_has_streams(wid, True)
    return ins