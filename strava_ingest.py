...
import database as db                         # <-- без точка
from strava_auth import refresh_access_token  # <-- без точка
from processing import bin_30s                # <-- без точка
...
def fetch_activities(athlete_key, after_ts=None, before_ts=None, client_id=None, client_secret=None):
    """
    Downloads activities list and inserts workout metadata into DB.
    Returns list of Strava activity IDs that were fetched.
    """
    ...
    # insert into DB (ignore the helper key)
...
def fetch_streams_for_activity(athlete_key, activity_id, client_id=None, client_secret=None, attach_workout_id=None):
    """
    Downloads 1 Hz streams for a specific activity and stores 30s binned HR–V points.
    ...
    # Find workout to attach (fallback to last)
