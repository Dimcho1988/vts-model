...
# ------------- hr_speed_points -------------
def insert_hr_speed_points(workout_id: int, points: List[Dict]):
    sql = """
...
def get_token(athlete_key: str):
    sql = """
    select athlete_key, strava_athlete_id, access_token, refresh_token, expires_at
    from tokens where athlete_key=:athlete_key limit 1
    """
    with get_engine().begin() as conn:
        row = conn.execute(text(sql), {"athlete_key": athlete_key}).mappings().first()
        return dict(row) if row else None
