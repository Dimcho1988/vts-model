...
def save_hrmax(engine, athlete_id: int, hrmax: int) -> None:
    ensure_profiles_schema(engine)
    with engine.begin() as c:
        c.execute(text("update user_profiles set hrmax=:hr, updated_at=now() where athlete_id=:aid"),
                  {"hr": int(hrmax), "aid": athlete_id})

def save_cs(engine, athlete_id: int, cs_kmh: float) -> None:
    ensure_profiles_schema(engine)
    with engine.begin() as c:
        c.execute(text("update user_profiles set cs_kmh=:cs, updated_at=now() where athlete_id=:aid"),
...
def _zones_abs_from_perc(zperc: List[Dict[str,Any]], cs_kmh: Optional[float]) -> List[Dict[str,Any]]:
    ...
    for z in zperc:
        ...
        out.append({"zone":z["zone"], "speed_low_kmh":round(lo,3), "speed_high_kmh":round(hi,3), "note":z.get("note","")})
    return out
...
def save_speed_zones_perc(engine, athlete_id: int, zones_df: pd.DataFrame, cs_kmh: Optional[float]) -> None:
    ...
    """)
    with engine.begin() as c:
        c.execute(sql, {"aid": athlete_id, "zperc": json.dumps(zperc), "zabs": json.dumps(zabs)})

def get_profile(engine, athlete_id: int) -> Dict[str, Any]:
    ensure_profiles_schema(engine)
    with engine.begin() as c:
        row = c.execute(text("select * from user_profiles where athlete_id=:aid"), {"aid": athlete_id}).mappings().first()
    if not row:
        with engine.begin() as c:
            c.execute(text("insert into user_profiles(athlete_id, speed_zone_perc) values (:aid, :z::jsonb) on conflict do nothing"),
...
def zones_df_from_profile(profile: Dict[str, Any], cs_kmh: Optional[float]) -> pd.DataFrame:
    ...
    if isinstance(zperc, str):
        try: zperc = json.loads(zperc)
        except Exception: zperc = DEFAULT_ZONE_PERC
    df = pd.DataFrame(zperc)
    if cs_kmh:
        df["speed_low_kmh"]  = (df["low_%CS"]/100.0)  * cs_kmh
        df["speed_high_kmh"] = (df["high_%CS"]/100.0) * cs_kmh
    return df[["zone","low_%CS","high_%CS","note"] + ([ "speed_low_kmh","speed_high_kmh"] if cs_kmh else [])]
